...
public class Main extends Activity {
	...
	boolean saveCoordsWhenOffline = false;
	SQLiteDatabase GPS = null;
	
	protected SQLiteDatabase openLocationDB()
	{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		this.saveCoordsWhenOffline = settings.getBoolean("saveLocationOffline", true);
		...
		locationListener = new LocationListener() {
			...
			public void onLocationChanged(Location location) {
				...
				try {
					...
					} else if(saveCoordsWhenOffline) {
						// /TBD: save not transmitted data to database to
						// database
						if(GPS==null)
							GPS = openLocationDB();
						GPS.execSQL(
								String.format("INSERT INTO Coordinates (Lat, Lon) VALUES ('%f', '%f');", 
										location.getLatitude(), location.getLongitude()));
						Log.d("ME", "New record was inserted into DB");
					}
				} catch (Exception e) {
